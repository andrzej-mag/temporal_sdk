-module(temporal_sdk_test_replay_pe3_activity_tests).
...
-define(TESTS, [
    {timeout, 20, fun noevent/0},
    {timeout, 20, fun not_awaited/0},
    {timeout, 20, fun base/0},
    {timeout, 20, fun base_nde_1/0},
    {timeout, 20, fun base_nde_2/0},
    {timeout, 20, fun noawait_fail/0},
    {timeout, 20, fun complete/0},
    {timeout, 20, fun complete_nde/0},
    {timeout, 20, fun complete_prohibited_nde/0},
    {timeout, 20, fun cancel/0},
    {timeout, 20, fun cancel_nde/0},
    {timeout, 20, fun cancel_prohibited_nde/0},
    {timeout, 20, fun fail/0},
    {timeout, 20, fun fail_nde/0},
    {timeout, 20, fun fail_prohibited_nde/0},
    {timeout, 20, fun duplicate_id/0},
    {timeout, 20, fun throw_1/0},
    {timeout, 20, fun throw_2/0},
    {timeout, 20, fun throw_12/0},
    {timeout, 20, fun throw_21/0},
    {timeout, 20, fun throw_nde_1/0},
    {timeout, 20, fun throw_nde_2/0},
    {timeout, 20, fun loop/0},
    {timeout, 20, fun loop_nde_1/0},
    {timeout, 20, fun loop_nde_2/0},
    {timeout, 20, fun a_cancel_1/0},
    {timeout, 20, fun a_cancel_2/0},
    {timeout, 20, fun a_cancel_3/0},
    {timeout, 20, fun a_cancel_3_nde1/0},
    {timeout, 20, fun a_cancel_3_nde2/0},
    {timeout, 20, fun a_cancel_4/0},
    {timeout, 20, fun a_cancel_5/0},
    {timeout, 20, fun a_cancel_6/0},
    {timeout, 29, fun a_cancel_loop/0},
    % {timeout, 20, fun invalid_mod/0},
    {timeout, 20, fun long_fail_await/0},
    {timeout, 20, fun a_a/0},
    {timeout, 20, fun a_a_err/0},
    {timeout, 20, fun a_err_a/0},
    {timeout, 20, fun err_a_a/0},
    {timeout, 20, fun a_failing_1/0},
    {timeout, 20, fun a_failing_2/0},
    {timeout, 20, fun a_failing_3/0},
    {timeout, 20, fun large_data/0}
]).

-define(OPTS, []).
-define(LPATH, [json, pe3, activity, ?CONFIG_NAME]).

base_test_() -> ?FIXTURE(?CONFIGS, {inparallel, {timeout, 30, ?TESTS}}).

-define(MAIN_EXEC, fun(#{is_replaying := IsReplaying}, _Input) ->
    A1 = start_awaitables(),
    set_info(A1, [{info_id, a_root}]),
    A2 = start_awaitables(),
    Exe_1 = start_execution(?MODULE, execution_1, [?PEFN, IsReplaying], [{execution_id, exe_1}]),

    AWExe_1 = wait({info, aw_exe_1}, 1_000),
    assert_awaitables(AWExe_1),
    AWExe_2 = wait({info, aw_exe_2}, 5_000),
    assert_awaitables(AWExe_2),
    #{result := "execution_1_result"} = wait(Exe_1),
    #{result := "execution_2_result"} = wait({execution, exe_2}),
    assert_awaitables(A2),
    "execution_main_result"
end).

execution_1(_Context, [PEFN, IsReplaying]) ->
    A1 = start_awaitables(),
    set_info(A1, [{info_id, aw_exe_1}]),
    A2 = start_awaitables(),
    start_execution(?MODULE, PEFN, [IsReplaying], [{execution_id, pef}]),

    ARoot = wait({info, a_root}),
    assert_awaitables(ARoot),
    assert_awaitables(A2),
    start_execution(?MODULE, execution_2, [IsReplaying], [{execution_id, exe_2}]),
    "execution_1_result".

execution_2(_Context, _Input) ->
    A1 = start_awaitables(),
    set_info(A1, [{info_id, aw_exe_2}]),
    "execution_2_result".

start_awaitables() ->
    [
        start_activity(?A_TYPE, ["regular_execution"], []),
        start_activity(?A_TYPE, ["session_execution"], [session_execution]),
        start_activity(?A_TYPE, ["eager_execution"], [eager_execution]),
        % start_activity(?A_TYPE, ["direct_execution"], [direct_execution]),
        % start_activity(?A_TYPE, ["direct_result"], [direct_result]),
        record_marker(fun() -> ["regular marker"] end, [])
    ].
assert_awaitables(Awaitables) ->
    ?assertMatch(
        [
            #{result := ["regular_execution"]},
            #{result := ["session_execution"]},
            #{result := ["eager_execution"]},
            % #{result := ["direct_execution"]},
            % #{result := ["direct_result"]},
            #{value := ["regular marker"]}
        ],
        wait_all(Awaitables)
    ).
