...
convert('temporal.api.activity.v1.ActivityExecutionInfo' = N, M, Cl, RI, CC, C) ->
    P = [
...
convert('temporal.api.activity.v1.ActivityExecutionListInfo' = N, M, Cl, RI, CC, C) ->
    P = [{search_attributes, {optional, 'temporal.api.common.v1.SearchAttributes'}}],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.activity.v1.ActivityExecutionOutcome' = N, M, Cl, RI, CC, C) ->
    P = [
        {{value, result}, {optional, 'temporal.api.common.v1.Payloads'}},
        {{value, failure}, {optional, 'temporal.api.failure.v1.Failure'}}
    ],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.activity.v1.CallbackInfo' = N, M, Cl, RI, CC, C) ->
    P = [{info, {optional, 'temporal.api.callback.v1.CallbackInfo'}}],
    convert_paths(P, N, M, Cl, RI, CC, C);
...
convert('temporal.api.batch.v1.BatchOperationTermination' = N, M, Cl, RI, CC, C) ->
    ...
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.callback.v1.CallbackInfo' = N, M, Cl, RI, CC, C) ->
...
convert('temporal.api.common.v1.SearchAttributes' = N, M, Cl, RI, CC, C) ->
    ...
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.compute.v1.ComputeConfig' = N, M, Cl, RI, CC, C) ->
    P = [{scaling_groups, {map_string, 'temporal.api.compute.v1.ComputeConfigScalingGroup'}}],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.compute.v1.ComputeConfigScalingGroup' = N, M, Cl, RI, CC, C) ->
    P = [
        {provider, {optional, 'temporal.api.compute.v1.ComputeProvider'}},
        {scaler, {optional, 'temporal.api.compute.v1.ComputeScaler'}}
    ],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.compute.v1.ComputeConfigScalingGroupUpdate' = N, M, Cl, RI, CC, C) ->
    P = [{scaling_group, {optional, 'temporal.api.compute.v1.ComputeConfigScalingGroup'}}],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.compute.v1.ComputeProvider' = N, M, Cl, RI, CC, C) ->
    P = [{details, {optional, 'temporal.api.common.v1.Payload'}}],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.compute.v1.ComputeScaler' = N, M, Cl, RI, CC, C) ->
    P = [{details, {optional, 'temporal.api.common.v1.Payload'}}],
    convert_paths(P, N, M, Cl, RI, CC, C);
...
convert('temporal.api.deployment.v1.WorkerDeploymentVersionInfo' = N, M, Cl, RI, CC, C) ->
    P = [
        {metadata, {optional, 'temporal.api.deployment.v1.VersionMetadata'}},
        {compute_config, {optional, 'temporal.api.compute.v1.ComputeConfig'}}
    ],
    convert_paths(P, N, M, Cl, RI, CC, C);
...
convert('temporal.api.nexus.v1.EndpointSpec' = N, M, Cl, RI, CC, C) ->
    ...
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.nexus.v1.NexusOperationExecutionCancellationInfo' = N, M, Cl, RI, CC, C) ->
    P = [{last_attempt_failure, {optional, 'temporal.api.failure.v1.Failure'}}],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.nexus.v1.NexusOperationExecutionInfo' = N, M, Cl, RI, CC, C) ->
    P = [
        {search_attributes, {optional, 'temporal.api.common.v1.SearchAttributes'}},
        {last_attempt_failure, {optional, 'temporal.api.failure.v1.Failure'}},
        {user_metadata, {optional, 'temporal.api.sdk.v1.UserMetadata'}},
        {cancellation_info,
            {optional, 'temporal.api.nexus.v1.NexusOperationExecutionCancellationInfo'}}
    ],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.nexus.v1.NexusOperationExecutionListInfo' = N, M, Cl, RI, CC, C) ->
    P = [{search_attributes, {optional, 'temporal.api.common.v1.SearchAttributes'}}],
    convert_paths(P, N, M, Cl, RI, CC, C);
...
convert('temporal.api.nexus.v1.StartOperationResponse' = N, M, Cl, RI, CC, C) ->
    P = [
        {{variant, failure}, {optional, 'temporal.api.failure.v1.Failure'}},
...
convert('temporal.api.workflow.v1.WorkflowExecutionInfo' = N, M, Cl, RI, CC, C) ->
    ...
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.workflowservice.v1.CountActivityExecutionsResponse' = N, M, Cl, RI, CC, C) ->
    P = [
        {groups,
            {repeated,
                'temporal.api.workflowservice.v1.CountActivityExecutionsResponse.AggregationGroup'}}
    ],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert(
    'temporal.api.workflowservice.v1.CountActivityExecutionsResponse.AggregationGroup' = N,
    M,
    Cl,
    RI,
    CC,
    ...
    convert_paths(P, N, M, Cl, RI, CC, C);
convert(
    'temporal.api.workflowservice.v1.CountNexusOperationExecutionsResponse' = N, M, Cl, RI, CC, C
) ->
    P = [
        {groups,
            {repeated,
    ...
    convert_paths(P, N, M, Cl, RI, CC, C);
convert(
    'temporal.api.workflowservice.v1.CountNexusOperationExecutionsResponse.AggregationGroup' = N,
    M,
    Cl,
    RI,
    CC,
    ...
    P = [{group_values, {repeated, 'temporal.api.common.v1.Payload'}}],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.workflowservice.v1.CountSchedulesResponse' = N, M, Cl, RI, CC, C) ->
    P = [
        {groups,
            {repeated, 'temporal.api.workflowservice.v1.CountSchedulesResponse.AggregationGroup'}}
    ],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert(
    'temporal.api.workflowservice.v1.CountSchedulesResponse.AggregationGroup' = N, M, Cl, RI, CC, C
) ->
    P = [{group_values, {repeated, 'temporal.api.common.v1.Payload'}}],
    convert_paths(P, N, M, Cl, RI, CC, C);
...
convert('temporal.api.workflowservice.v1.CreateScheduleRequest' = N, M, Cl, RI, CC, C) ->
    ...
    convert_paths(P, N, M, Cl, RI, CC, C);
convert(
    'temporal.api.workflowservice.v1.CreateWorkerDeploymentVersionRequest' = N, M, Cl, RI, CC, C
) ->
    P = [{compute_config, {optional, 'temporal.api.compute.v1.ComputeConfig'}}],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.workflowservice.v1.DescribeActivityExecutionResponse' = N, M, Cl, RI, CC, C) ->
    P = [
        {input, {optional, 'temporal.api.common.v1.Payloads'}},
        {info, {optional, 'temporal.api.activity.v1.ActivityExecutionInfo'}},
        {outcome, {optional, 'temporal.api.activity.v1.ActivityExecutionOutcome'}},
        {callbacks, {repeated, 'temporal.api.activity.v1.CallbackInfo'}}
    ],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.workflowservice.v1.DescribeDeploymentResponse' = N, M, Cl, RI, CC, C) ->
    ...
    convert_paths(P, N, M, Cl, RI, CC, C);
convert(
    'temporal.api.workflowservice.v1.DescribeNexusOperationExecutionResponse' = N, M, Cl, RI, CC, C
) ->
    P = [
        {input, {optional, 'temporal.api.common.v1.Payload'}},
        {{outcome, result}, {optional, 'temporal.api.common.v1.Payload'}},
        {{outcome, failure}, {optional, 'temporal.api.failure.v1.Failure'}},
        {info, {optional, 'temporal.api.nexus.v1.NexusOperationExecutionInfo'}}
    ],
    convert_paths(P, N, M, Cl, RI, CC, C);
...
convert(
    ...
    P = [{history, {optional, 'temporal.api.history.v1.History'}}],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.workflowservice.v1.ListActivityExecutionsResponse' = N, M, Cl, RI, CC, C) ->
    P = [{executions, {repeated, 'temporal.api.activity.v1.ActivityExecutionListInfo'}}],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert(
...
convert(
    ...
    convert_paths(P, N, M, Cl, RI, CC, C);
convert(
    'temporal.api.workflowservice.v1.ListNexusOperationExecutionsResponse' = N, M, Cl, RI, CC, C
) ->
    P = [{operations, {repeated, 'temporal.api.nexus.v1.NexusOperationExecutionListInfo'}}],
    convert_paths(P, N, M, Cl, RI, CC, C);
...
convert('temporal.api.workflowservice.v1.ListWorkflowExecutionsResponse' = N, M, Cl, RI, CC, C) ->
    P = [{executions, {repeated, 'temporal.api.workflow.v1.WorkflowExecutionInfo'}}],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.workflowservice.v1.PollActivityExecutionResponse' = N, M, Cl, RI, CC, C) ->
    P = [{outcome, {optional, 'temporal.api.activity.v1.ActivityExecutionOutcome'}}],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.workflowservice.v1.PollActivityTaskQueueResponse' = N, M, Cl, RI, CC, C) ->
    ...
    convert_paths(P, N, M, Cl, RI, CC, C);
convert(
    'temporal.api.workflowservice.v1.PollNexusOperationExecutionResponse' = N, M, Cl, RI, CC, C
) ->
    P = [
        {{outcome, result}, {optional, 'temporal.api.common.v1.Payload'}},
        {{outcome, failure}, {optional, 'temporal.api.failure.v1.Failure'}}
...
convert('temporal.api.workflowservice.v1.RespondNexusTaskCompletedRequest' = N, M, Cl, RI, CC, C) ->
    ...
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.workflowservice.v1.RespondNexusTaskFailedRequest' = N, M, Cl, RI, CC, C) ->
    P = [{failure, {optional, 'temporal.api.failure.v1.Failure'}}],
    convert_paths(P, N, M, Cl, RI, CC, C);
...
convert('temporal.api.workflowservice.v1.SignalWorkflowExecutionRequest' = N, M, Cl, RI, CC, C) ->
    ...
    convert_paths(P, N, M, Cl, RI, CC, C);
convert('temporal.api.workflowservice.v1.StartActivityExecutionRequest' = N, M, Cl, RI, CC, C) ->
    P = [
        {input, {optional, 'temporal.api.common.v1.Payloads'}},
        {search_attributes, {optional, 'temporal.api.common.v1.SearchAttributes'}},
        {header, {optional, 'temporal.api.common.v1.Header'}},
...
convert('temporal.api.workflowservice.v1.StartBatchOperationRequest' = N, M, Cl, RI, CC, C) ->
    ...
    convert_paths(P, N, M, Cl, RI, CC, C);
convert(
    'temporal.api.workflowservice.v1.StartNexusOperationExecutionRequest' = N, M, Cl, RI, CC, C
) ->
    P = [
        {input, {optional, 'temporal.api.common.v1.Payload'}},
        {search_attributes, {optional, 'temporal.api.common.v1.SearchAttributes'}},
...
convert('temporal.api.workflowservice.v1.UpdateScheduleRequest' = N, M, Cl, RI, CC, C) ->
    ...
    convert_paths(P, N, M, Cl, RI, CC, C);
convert(
    'temporal.api.workflowservice.v1.UpdateWorkerDeploymentVersionComputeConfigRequest' = N,
    M,
    Cl,
    RI,
    CC,
    ...
    P = [
        {compute_config_scaling_groups,
            {map_string, 'temporal.api.compute.v1.ComputeConfigScalingGroupUpdate'}}
    ],
    convert_paths(P, N, M, Cl, RI, CC, C);
convert(
...
convert('temporal.api.workflowservice.v1.UpdateWorkflowExecutionResponse' = N, M, Cl, RI, CC, C) ->
    ...
    convert_paths(P, N, M, Cl, RI, CC, C);
convert(
    'temporal.api.workflowservice.v1.ValidateWorkerDeploymentVersionComputeConfigRequest' = N,
    M,
    Cl,
    RI,
    CC,
